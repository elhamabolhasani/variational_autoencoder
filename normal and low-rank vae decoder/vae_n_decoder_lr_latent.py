...
class VAE(nn.Module):
    def __init__(self, dataset):
        ...
        # model name
        self.model_name = dataset + 'vae_n_decoder_lr_latent'
    ...
    def sampling(self):
        ...
        mu_d, logvar_d = self.decoder_bottleneck(z)
        # sigma = torch.exp(logvar_d)
        # mat_std = torch.diag_embed(sigma)
        # dist = torch.distributions.multivariate_normal.MultivariateNormal(mu_d, mat_std)
    ...
    def forward(self, x):
        ...
        return mu_d, logvar_d, mu, logvar, u
    ...
    def load_data(self, dataset):
        ...
        if dataset == "mnist":
            ...
            train_loader = torch.utils.data.DataLoader(train, batch_size=64, shuffle=True, num_workers=0)
            test_loader = torch.utils.data.DataLoader(test, batch_size=64, shuffle=True, num_workers=0)

        return train_loader, test_loader

    def loss_function(self, mu_d, logvar_d, x, mu, logvar, u):
        # BCD
        # print(mu_d.shape)
        # print(x.shape)
        # print(logvar_d.shape)
        # one
        # sigma = torch.exp(logvar_d)
        # mat_std = torch.diag_embed(sigma)
        # dist = torch.distributions.multivariate_normal.MultivariateNormal(mu_d, mat_std)
        ...
        # print('refoem x shape ', x.view(-1, x.shape[2] * x.shape[2]).shape)
    ...
    def fit_train(self, epoch):
        ...
        for batch_idx, (inputs, _) in enumerate(self.train_loader):
            ...
            mu_d, logvar_d, mu, logvar, u = self(inputs)

            loss, z = self.loss_function(mu_d, logvar_d, inputs, mu, logvar, u)
...
train_phase = True
model_path = '/checkpoints/vae_n_decoder_lr_latent_cifar_model.pt'
optim_path = '/checkpoints/vae_n_decoder_lr_latent_cifar_optim.pt'
model_name = 'mnist'
if train_phase:
    net = VAE(model_name)
    net.init_model()
