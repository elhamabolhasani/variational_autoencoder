...
class VAE(nn.Module):
    ...
    def loss_function(self, mu_d, logvar_d, x, mu, logvar):
        dist = Independent(Normal(loc=mu_d, scale=torch.exp(logvar_d)), 1)
    ...
    def fit_train(self, epoch):
        ...
        for batch_idx, (inputs, _) in enumerate(self.train_loader):
            ...
            mu_d, logvar_d, mu, logvar = self(inputs)

            loss, z = self.loss_function(mu_d, logvar_d, inputs, mu, logvar)
...
train_phase = True
model_path = './checkpoints/vae_n_decoder_n_latent_cifar_model_0501.pt'
optim_path = './checkpoints/vae_n_decoder_n_latent_cifar_optim_0501.pt'
model_name = 'cifar'
if train_phase:
    net = VAE(model_name)
    net.init_model()
    for i in range(0, 1010):
        ...
        if i % 100 == 0 and i != 0:
            torch.save(
                net.state_dict(),
                "./checkpoints/vae_n_decoder_n_latent_dim=256_" + model_name + f"_model_{str(i + 1).zfill(4)}.pt"
            )
            torch.save(
                net.optimizer.state_dict(),
                "./checkpoints/vae_n_decoder_n_latent_dim" + model_name + f"_optim_{str(i + 1).zfill(4)}.pt"
            )
    ...
    net.eval()
    save_image(net.sampling(), "test.png", nrow=8)
