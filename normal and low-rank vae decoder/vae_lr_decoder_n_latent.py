...
class VAE(nn.Module):
    def __init__(self, dataset):
        ...
        # model name
        self.model_name = dataset + '_vae_lr_decoder_n_latent_sigma=0.001'
    ...
    def forward(self, x):
        ...
        return mu_d, logvar_d, u_d, mu, logvar
    ...
    def loss_function(self, mu_d, logvar_d, u_d, x, mu, logvar):
        # print(mu_d.shape)
        # print(x.shape)
    ...
    def fit_train(self, epoch):
        ...
        for batch_idx, (inputs, _) in enumerate(self.train_loader):
            ...
            mu_d, logvar_d, u_d, mu, logvar = self(inputs)
            loss, z = self.loss_function(mu_d, logvar_d, u_d, inputs, mu, logvar)
...
train_phase = True
model_path = '/checkpoints/.pt'
optim_path = '/checkpoints/.pt'
model_name = 'cifar'
if train_phase:
    net = VAE(model_name)
    net.init_model()
    for i in range(0, 400):
        ...
        if i % 100 == 0 and i != 0:
            torch.save(
                net.state_dict(),
                "./checkpoints/vae_lr_decoder_n_latent_" + model_name + f"_model_{str(i + 1).zfill(4)}.pt"
            )
            torch.save(
                net.optimizer.state_dict(),
                "./checkpoints/vae_lr_decoder_n_latent_" + model_name + f"_optim_{str(i + 1).zfill(4)}.pt"
            )
